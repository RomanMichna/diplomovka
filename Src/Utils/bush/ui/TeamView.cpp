...
void TeamView::init()
{
  if(team)
  {
    ...
    /*
    handicapSlider = new QSlider(Qt::Horizontal, this);
    handicapSlider->setSliderPosition(getHandicap() * 100);
    handicapSlider->setRange(1,100);
    handicapSlider->setFixedWidth(120);
    settingsGrid->addWidget(new QLabel("Handicap:"), 0, 10);
    settingsGrid->addWidget(handicapSlider, 0, 11);
    handiReadOut = new QLabel("");
    handiReadOut->setNum(getHandicap() * 100);
    handiReadOut->setFixedWidth(25);
    settingsGrid->addWidget(handiReadOut, 0, 12);
    connect(handicapSlider, SIGNAL( valueChanged( int ) ), handiReadOut, SLOT( setNum( int ) ) );
    connect(handicapSlider, SIGNAL( valueChanged( int ) ), this, SLOT(handicapChanged(const int)));
    */
...
/*
void TeamView::handicapChanged(const int handicap)
{
  try
  {
    ConfigMap cb;
    
    if(cb.read("Locations/" + team->location + "/behavior2012Parameters.cfg", false, &ConfigMap::printOnErr) >= 0)
    {
      cb.setFlags(cb.getFlags() | ConfigMap::READONLY);
      cb["handicap"] << ((float)handicap / 100.f);
      std::ofstream of;
      of.open(("Locations/" + team->location + "/behavior2012Parameters.cfg").c_str());
      if(!of.is_open())
        return;
      cb.write(of);
    }
