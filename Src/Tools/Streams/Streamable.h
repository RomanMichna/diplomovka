...
namespace Streaming
{
  ...
  */
  ...
  /**
  * The following set of function signatures are used to determine whether a variable is of an enum
  * type. For enums the signature with an int parameter will be picked that has a different return
  * type. Note that using these functions assumes that the variable tested is not directly of type int.
