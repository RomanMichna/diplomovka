...
{
private:
  std::tr1::unordered_map< std::string, DebugDataHeader*> table;
...
public:
  ~DebugDataTable()
  {
    std::tr1::unordered_map< std::string, DebugDataHeader*>::iterator iter;
    iter = table.begin();
  ...
  {
    std::tr1::unordered_map< std::string, DebugDataHeader*>::iterator iter = table.find(name);
  ...
  bool processChangeRequest(In& in)
  {
    ...
    {
      for(
        std::tr1::unordered_map< std::string, DebugDataHeader*>::iterator iter = table.begin();
        iter != table.end();
        ++iter
      )
      {
