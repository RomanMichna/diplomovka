...
#define DEBUG_IMAGE_SET_PIXEL_YUV(id, xx, yy, Y, U, V) \
  if(((int)(xx))+1 > 0 && ((int)(xx)) < id##Image.resolutionWidth && \
     ((int)(yy))+1 > 0 && ((int)(yy)) < id##Image.resolutionHeight) \
  { \
...
#define INIT_DEBUG_IMAGE_BLACK(id) \
  ...
  { \
    for (int y = 0; y < id##Image.resolutionHeight; y++) \
      for (int x = 0; x < id##Image.resolutionWidth; x++) \
