...
public:
  ...
  */
  template <typename T> static void grahamScan(vector<T>& points)
  {
    ...
    typename vector<T>::iterator start = min_element(points.begin(), points.end(), lexicographicalComparator<T>);
    ...
    vector<T> stack(points.begin(), points.begin() + 2);

    for(typename vector<T>::iterator i = points.begin() + 2; i != points.end(); ++i)
    {
  ...
  * @param points The set of points.
  */
  template <class T> static void jarvisMarch(vector<T>& points)
  {
    // if the set of points consists of two points or less, they build the convex hull
    if(points.size() <= 2)
    {
    ...
    // find a point that is part of the convex hull
    typename vector<T>::iterator start = min_element(points.begin(), points.end(), lexicographicalComparator<T>);
    typename vector<T>::iterator next = start;
