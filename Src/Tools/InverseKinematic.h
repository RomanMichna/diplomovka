...
public:
  ...
  */
  static bool calcLegJoints(const Pose3D& positionLeft, const Pose3D& positionRight, JointData& jointData, const RobotDimensions& robotDimensions, float ratio = 0.5f)
  {
    ...
    const float lMinusJoint5 = atan2(lFootToHip.y, lFootToHip.z);
    const float rJoint5 = atan2(rFootToHip.y, rFootToHip.z);
    const float lMinusBetaAndJoint4 = -atan2(lFootToHip.x, sqrt(sqr(lFootToHip.y) + sqr(lFootToHip.z)));
    const float rMinusBetaAndJoint4 = -atan2(rFootToHip.x, sqrt(sqr(rFootToHip.y) + sqr(rFootToHip.z)));
    ...
    const float lMinusJoint0 = atan2(-lHipRotationC1.x, lHipRotationC1.y);
    const float rJoint0 = atan2(-rHipRotationC1.x, rHipRotationC1.y);
    ...
    const float lMinusPi_4MinusJoint1 = -atan2(-lHipToFoot.y, -lHipToFoot.z);
    const float rPi_4AndJoint1 = -atan2(-rHipToFoot.y, -rHipToFoot.z);
    const float lJoint2MinusAlpha = atan2(-lHipToFoot.x, sqrt(sqr(lHipToFoot.y) + sqr(lHipToFoot.z)) * -sgn(lHipToFoot.z));
    const float rJoint2MinusAlpha = atan2(-rHipToFoot.x, sqrt(sqr(rHipToFoot.y) + sqr(rHipToFoot.z)) * -sgn(rHipToFoot.z));
    ...
    const float lAlpha = -acos(cosClipping.limit(lCosMinusAlpha));
    const float rAlpha = -acos(cosClipping.limit(rCosMinusAlpha));
    const float lBeta = -acos(cosClipping.limit(lCosMinusBeta));
    const float rBeta = -acos(cosClipping.limit(rCosMinusBeta));
    ...
    jointData.angles[JointData::LHipYawPitch + 4] = atan2(lFootRotationC2.x, lFootRotationC2.z) + lBeta;
    jointData.angles[JointData::LHipYawPitch + 5] = -asin(-lFootRotationC2.y);
    ...
    jointData.angles[JointData::RHipYawPitch + 4] = atan2(rFootRotationC2.x, rFootRotationC2.z) + rBeta;
    jointData.angles[JointData::RHipYawPitch + 5] = asin(-rFootRotationC2.y);
    /*
...
//      ASSERT(cosAngle > -1.1);
  ...
  */
  ...
  static void calcHeadJoints(const Vector3<>& position, const float imageTilt,
                             const RobotDimensions& robotDimensions, const bool lowerCamera, Vector2<>& panTilt)
  {
    Vector2<> headJoint2Target(sqrt(sqr(position.x) + sqr(position.y)), position.z - robotDimensions.zLegJoint1ToHeadPan);
    ...
    panTilt.y = -tilt;
    panTilt.x = atan2(position.y, position.x);
  }
};
