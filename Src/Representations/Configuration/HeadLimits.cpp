...
HeadLimits& HeadLimits::operator=(const HeadLimits& other)
{
  shoulderRadius = other.shoulderRadius;
                                              ...
                                              Vector3<>& intersection) const
{
  ...
  if(abs(intersection.x) > shoulderRadius)
    return false; // No intersection with the shoulder circle.
  intersection.z = shoulderRadius * sin(acos(intersection.x / shoulderRadius));
                                               ...
                                               const float imageTilt, Vector3<>& intersection) const
{
  ...
  Vector3<> line(cos(-imageTilt), 0.0f, sin(-imageTilt));
