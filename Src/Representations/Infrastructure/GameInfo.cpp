...
void GameInfo::draw()
{
  DECLARE_DEBUG_DRAWING3D("representation:GameInfo", "field",
  {
    ...
    drawDigit(mins / 10, Vector3<>(-350, 2500, 1000), 200, color);
    drawDigit(mins % 10, Vector3<>(-80, 2500, 1000), 200, color);
    drawDigit(secs / 10, Vector3<>(280, 2500, 1000), 200, color);
    drawDigit(secs % 10, Vector3<>(550, 2500, 1000), 200, color);
    LINE3D("representation:GameInfo", 0, 2500, 890, 0, 2500, 910, 3, color);
    LINE3D("representation:GameInfo", 0, 2500, 690, 0, 2500, 710, 3, color);
  });

  DECLARE_DEBUG_DRAWING("representation:GameInfo","drawingOnField",
  {
    DRAWTEXT("representation:GameInfo", -3500, -2200, 14, ColorClasses::white, "Time remaining: " << (int)(secsRemaining/60) << ":" << (secsRemaining%60));
    DRAWTEXT("representation:GameInfo", -3500, -2400, 14, ColorClasses::white, (firstHalf ? "First" : "Second") << " half");
    std::string stateStr;
    switch(state)
    {
    case STATE_INITIAL:
