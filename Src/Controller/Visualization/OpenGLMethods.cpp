...
)
{
  ...
  for(int j = y1; j < y2 - 2; j++)
  {
    for(int i = x1; i < x2 - 2; i++)
    {
      ...
      {
        x = (float)(+i - image.resolutionWidth / 2) * tmp;
        y = (float)(-j + image.resolutionHeight / 2) * tmp;
        xn = (float)(+i + 1 - image.resolutionWidth / 2) * tmp;
        yn = (float)(-j - 1 + image.resolutionHeight / 2) * tmp;
