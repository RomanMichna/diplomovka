...
bool TeamComm3DCtrl::compile()
{
  ...
  for(unsigned i = 0, count = application->getObjectChildCount(*group); i < count; ++i)
  {
    ...
    puppetData.name = name.toAscii().constData();
  ...
  for(int teamColor = firstTeamColor; teamColor < numOfTeamColors; ++teamColor)
    for(int robotNumber = TeamMateData::firstPlayer; robotNumber < TeamMateData::numOfPlayers; ++robotNumber)
    {
      ...
      if(puppetData.robot)
      {
        ...
        if(!robotData || now - robotData->timeStamp > TeamDataProvider::networkTimeout || robotData->isPenalized)
        {
          ...
          if(puppetData.online)
          {
            ...
            for(std::tr1::unordered_map<const char*, DebugDrawing3D>::iterator i = puppetData.drawings3D.begin(), end = puppetData.drawings3D.end(); i != end; ++i)
              i->second.reset();
        ...
        else
        {
          ...
          if(puppetData.updateTimeStamp != robotData->timeStamp || !robotData->hasGroundContact)
          {
            ...
            const float* pos = ((SimRobotCore2::Body*)puppetData.robot)->getPosition();
            puppetData.oracle.moveRobot(
                Vector3<>(robotPose.translation.x, robotPose.translation.y, robotData->hasGroundContact ? (pos[2] > 0.28f ? (pos[2] * 1000.f) :  puppetData.initialPosition.z) : puppetData.initialPosition.z + 600.f),
...
bool TeamComm3DCtrl::handleMessage(InMessage& message)
{
  ...
  case idTeamMateObstacleModel:
  {
    ...
    currentRobotData->obstacleModel = theObstacleModelCompressed.unpack();
    return true;
  }
  case idSensorData:
