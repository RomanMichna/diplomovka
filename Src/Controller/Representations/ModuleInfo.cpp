...
std::string ModuleInfo::sendRequest(Out& stream, bool sort) const
{
    ...
    if(i->selected != "")
    {
      ...
      for(std::vector<std::string>::const_iterator k = j->requirements.begin(); k != j->requirements.end(); ++k)
      {
        ...
        if(m == providers.end())
        {
          ...
          else if(std::find(config.sharedRepresentations.begin(), config.sharedRepresentations.end(), *k) == config.sharedRepresentations.end())
            config.sharedRepresentations.push_back(*k);
        }
      }
    }
  }
  ...
  if(sort)
  {
    std::sort(config.sharedRepresentations.begin(), config.sharedRepresentations.end());
