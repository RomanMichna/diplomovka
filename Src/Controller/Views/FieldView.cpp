...
private:
  ...
  void paintDrawings(QPainter& painter)
  {
    ...
    {
      const DebugDrawing& debugDrawing(fieldView.console.fieldDrawings[*i]);
  ...
  void mouseMoveEvent(QMouseEvent* event)
  {
    ...
    {
      SYNC_WITH(fieldView.console);
      if(!fieldView.console.colorTableHandler.active)
      {
        ...
        for(std::list<std::string>::const_iterator i = drawings.begin(), end = drawings.end(); i != end; ++i)
        {
          text = fieldView.console.fieldDrawings[*i].getTip(pos.rx(), pos.ry());
          if(text)
            break;
        }
        if(text)
  ...
  virtual QMenu* createUserMenu() {return new QMenu(tr("&Field"));}
