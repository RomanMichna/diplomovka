...
bool Joystick::update()
{
  ...
  if(hatId)
  {
    ...
    int dir = IOHIDValueGetIntegerValue(valueRef) - 1;
    newState = dir >= 0 ? 1 << dir : 0;
    buttonEvents[1] |= buttonState[1] ^ newState;
    buttonState[1] = newState;
    if(dir >= 0)
    {
      axisState[0] = axisState[1] = 0;
      if(dir >= 7 || dir <= 1)
        axisState[1] = axisMin[1];
      else if(dir >= 3 && dir <= 5)
        axisState[1] = axisMax[1];
      if(dir >= 1 && dir <= 3)
        axisState[0] = axisMax[0];
      else if(dir >= 5 && dir <= 7)
        axisState[0] = axisMin[0];
    }
  }
