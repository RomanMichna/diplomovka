...
void WalkingEngine::updateMotionRequest()
{
  ...
  if(theInertiaSensorData.calibrated && theGroundContactState.contact && theMotionSelection.ratios[MotionRequest::walk] > 0.999f && !instable)
    if(theMotionRequest.motion == MotionRequest::walk)
    {
      bool allowWalking = true;
      if(currentMotionType == stand)
      {
        Vector3<> angleDiff = ((const RotationMatrix&)(p.standBodyRotation.invert() * theTorsoMatrix.rotation)).getAngleAxis();
        if(angleDiff.squareAbs() >= 0.0001f)
          allowWalking = false;
      }
      if(allowWalking)
      {
...
{
  if(theGroundContactState.contact && theFallDownState.state != FallDownState::onGround && theFallDownState.state != FallDownState::falling)
  {
