...
{
  MODIFY("parameters:CombinedWorldModelProvider:parameters", parameters);
...
void CombinedWorldModelProvider::updateOthers(CombinedWorldModel& combinedWorldModel)
{
  ...
  for(int i = TeamMateData::firstPlayer; i < TeamMateData::numOfPlayers; i++)
  {
    ...
    //if robot is penalized, not upright or has no ground contact
    if(i != theRobotInfo.number && (theOwnTeamInfo.players[i - 1].penalty != PENALTY_NONE || !theTeamMateData.isUpright[i] || theTeamMateData.isPenalized[i] || !theTeamMateData.hasGroundContact[i] || theFrameInfo.getTimeSince(theTeamMateData.timeStamps[i]) > TeamDataProvider::networkTimeout))
    {
      if(!oldOthersBallModelUsed[i])// if old BallModel was not already saved
      {
        ...
        {
          if(theFrameInfo.getTimeSince(ballModelsAllOtherPlayers[i][ringBufferIndex].timeWhenLastSeen) > OthersParameters.ballModelAge) // save the first BallModel which is older than x sec.
          {
            break;
          }
        }
...
BallState CombinedWorldModelProvider::getCombinedBallPosition(bool& ballIsValid)
{
  ...
  for(int i = TeamMateData::firstPlayer; i < TeamMateData::numOfPlayers; i++)
  {
    ...
    if((i == theRobotInfo.number && (fabs((theRobotPose * lastValidBallModel[i].estimate.position).x) > (float)theFieldDimensions.xPosOpponentFieldBorder || fabs((theRobotPose * lastValidBallModel[i].estimate.position).y) > (float)theFieldDimensions.yPosLeftFieldBorder))
      || (i != theRobotInfo.number && (fabs((theTeamMateData.robotPoses[i] * lastValidBallModel[i].estimate.position).x) > (float)theFieldDimensions.xPosOpponentFieldBorder || fabs((theTeamMateData.robotPoses[i] * lastValidBallModel[i].estimate.position).y) > (float)theFieldDimensions.yPosLeftFieldBorder)))
    {
      weights[i] = 0;
...
BallState CombinedWorldModelProvider::getCombinedBallPositionOthers(bool& ballIsValid, float& maxSideConfidence)
{
  ...
  for(int i = TeamMateData::firstPlayer; i < TeamMateData::numOfPlayers; i++)
  {
    ...
    if((i == theRobotInfo.number) || 
       ...
       (i != theRobotInfo.number && (fabs((theTeamMateData.robotPoses[i] * theTeamMateData.ballModels[i].estimate.position).x) > (float)theFieldDimensions.xPosOpponentFieldBorder || fabs((theTeamMateData.robotPoses[i] * theTeamMateData.ballModels[i].estimate.position).y) > (float)theFieldDimensions.yPosLeftFieldBorder)) ||
       (theFrameInfo.getTimeSince(theTeamMateData.ballModels[i].timeWhenLastSeen) > parameters.ballModelOthersTimeOut))
    {
      weights[i] = 0;
    }
    else
    {
      if(i != theRobotInfo.number)
      {
        ...
        maxSideConfidence = theTeamMateData.robotsSideConfidence[i].sideConfidence;
      }
    }
  }
...
float CombinedWorldModelProvider::computeWeights(const ExtendedBallModel& ballModel, const RobotPose& robotPose, unsigned timeWhenBallDisappeared) const
{
  ...
  weight *= 1.0f - (1.0f / (1.0f + expf(-(theFrameInfo.getTimeSince(timeWhenBallDisappeared) - parameters.movementFactorBallDisappeared) / parameters.scalingFactorBallDisappeared)));  // sigmoid function based on ball_time_when_disappeared

  const float angleOfCamera = atanf(ballModel.estimate.position.abs() / ballModel.cameraHeight); // angle of camera when looking at the ball
...
vector<GaussianDistribution> CombinedWorldModelProvider::getPositionOfOpponentRobots()
{
  ...
  for(std::vector<Cluster>::const_iterator i = allCluster.begin(); i != allCluster.end(); ++i) // calculates the merged position for each cluster
  {
    ...
    if(parameters.closeRobotsNeedLocalDetection)
    {
      if(detectedByMe || ((mean - theRobotPose.translation).abs() > parameters.closeRobotDetectionDistance))
