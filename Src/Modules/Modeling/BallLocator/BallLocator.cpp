...
BallLocator::BallLocator() : timeNotSeen(100000), firstDisappearance(0)
{
  ...
  p.ballNotSeenTimeout = 200;
  p.ballNotUpdatedTimeout = 8000;
}
...
void BallLocator::update(BallModel& ballModel)
{
  ...
  EXECUTE_ONLY_IN_DEBUG(ballModel.drawEndPosition((float) theFieldDimensions.ballFriction););
}
...
void BallLocator::generateModel(BallModel& ballModel)
{
  ...
  // update timeWhenDisappeared
  {
    ...
    {
      const Vector3<> camera = theCameraMatrix.invert() * Vector3<>(theBallModel.estimate.position.x, theBallModel.estimate.position.y, (float) theFieldDimensions.ballRadius);
      if(camera.x > 1)
      {
        ...
        if((image - Vector2<>(theCameraInfo.opticalCenter.x, theCameraInfo.opticalCenter.y)).squareAbs() < sqr(theCameraInfo.resolutionHeight * 0.42f))
    ...
    if(timeNotSeen > p.ballNotSeenTimeout)
      ballModel.timeWhenDisappeared = firstDisappearance;
    else
      ballModel.timeWhenDisappeared = theFrameInfo.time - timeNotSeen;

    if(theFrameInfo.getTimeSince(theBallModel.timeWhenLastSeen) >= (int) p.ballNotUpdatedTimeout)
