...
private:
  vector<int> clusterCount;              /**< Count number of particels per cluster. */
  vector<int> freeClusterIndizes;        /**< Keep track of free cluster indizes. */
  ...
  vector< pair<int, int> > clusters; /**< A list of all clusters which really contain samples.*/
...
public:
  ...
  void calcPose(RobotPose& robotPose)
  {
    ...
    for(unsigned i = 0; i < clusterCount.size(); ++i)
    {
      ...
      {
        clusters.push_back(make_pair(i, clusterCount[i]));
      }
    }
    ...
    {
    case MERGE_RANDOM_TO_LARGEST:
      mergeRandomClusterToLargest();
      break;
    case MERGE_TWO_LARGEST:
      mergeTwoLagestClusters();
      break;
    case PARTICLE_BLEEDING:
      mergeRandomSampleToLargestCluster();
      break;
    default:
      break;
    }
  ...
  void calcPoseOfCluster(RobotPose& robotPose, unsigned int numOfCluster)
  {
    ...
    if(clusterSize)
    {
      ...
      robotPose = Pose2D(averageRotation, (float) xSum / clusterSize, (float) ySum / clusterSize);
      robotPose.validity = static_cast<float>(clusterSize) / this->samples.size();
    }
  }
  ...
  vector< pair<int, int> > getClusters() const
  {
    return clusters;
  }
...
private:
  void mergeRandomClusterToLargest()
  {
    ...
    int startIdx = random((short)clusterCount.size());
  ...
  void mergeRandomSampleToLargestCluster()
  {
    ...
    Sample& sMerge = this->samples.at(random((short)this->samples.size()));
    if(sMerge.cluster == maxClusterIdx)
      return;
    // Check, if sample is inside cluster:
    Vector2<int> aabbMin(numeric_limits<int>::max(), numeric_limits<int>::max()),
            aabbMax(numeric_limits<int>::min(), numeric_limits<int>::min());
    for(int i = 0; i < this->samples.size(); ++i)
    {
