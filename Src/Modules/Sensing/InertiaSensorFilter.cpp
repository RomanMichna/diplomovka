...
{
  p.processNoise = Vector2f(0.004f, 0.004f);
  p.accNoise = Vector3f(1.f, 1.f, 1.f);
  p.calculatedAccLimit = Vector2f(fromDegrees(20.f), fromDegrees(30.f));
...
void InertiaSensorFilter::readingUpdate(const Vector3f& reading)
{
  ...
  const Matrix2x3f& kalmanGain = readingsSigmaPointsCov.transpose() * (readingsCov + p.sensorCov).invert();
