...
{
  p.gyroNoise = Vector2f(0.004f, 0.004f);
  p.accNoise = Vector3f(1.f, 1.f, 1.f);
  p.relativeKinematicsNoise = Vector3f(0.01f, 0.01f, 0.01f);
  p.absoluteKinematicsNoise = Vector2f(0.1f, 0.1f);

  p.uprightThreshold = Vector2f(fromDegrees(20.f), fromDegrees(30.f));
  p.horizontalProcessNoise = Vector3f(0.03f, 0.03f, 0.03f);

  p.init();
}

void ExpInertiaSensorFilter3D::Parameters::init()
{
...
{
  x = State();
  cov.c[0].x = sqr(p.absoluteKinematicsNoise.x * 10.f);
  cov.c[1].y = sqr(p.absoluteKinematicsNoise.y * 10.f);
...
{
  MODIFY("module:ExpInertiaSensorFilter3D:parameters", p);
  ...
  bool isUpright = abs(safeRawAngle.x) < p.uprightThreshold.x && abs(safeRawAngle.y) < p.uprightThreshold.y;
  ...
  if(isUpright)
  {
    ...
    {
      const RotationMatrix offset = RotationMatrix::fromRotationZ(getRotationZ(useLeft ? leftOffset.rotation :  rightOffset.rotation)) * RotationMatrix(Vector3<>(theInertiaSensorData.gyro.x * theFrameInfo.cycleTime, theInertiaSensorData.gyro.y * theFrameInfo.cycleTime, 0));
      processUpdate(&offset, Vector3f(p.gyroNoiseSqr.x, p.gyroNoiseSqr.y, p.relativeKinematicsNoiseSqr.z));
    }
    else
      processUpdate(useLeft ? &leftOffset.rotation :  &rightOffset.rotation, p.relativeKinematicsNoiseSqr);
  }
  else
  {
    processUpdate(0, p.horizontalProcessNoiseSqr);
  }
...
void ExpInertiaSensorFilter3D::accReadingUpdate(const Vector3f& reading)
{
  ...
  const Matrix2x3f kalmanGain = accReadingsSigmaPointsCov.transpose() * (accReadingsCov + p.accSensorCov).invert();
...
void ExpInertiaSensorFilter3D::angleReadingUpdate(const Vector2f& reading)
{
  ...
  const Matrix2x2f kalmanGain = angleReadingsSigmaPointsCov.transpose() * (angleReadingsCov + p.angleSensorCov).invert();
...
float ExpInertiaSensorFilter3D::getRotationZ(const RotationMatrix& rot3d) const
{
  /*
